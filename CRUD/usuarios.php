...
$datos = array('username'=>'', 'nombre'=>'', 'password'=>'', 'rol'=>'', 'id'=>'');
$accion = 'insert';

include 'app/controllers/getUser.php';
...
<h2 align="center">ADMINISTRACIÓN DE USUARIOS</h2>
	  ...
	  <form align="center" action="postUsuario.php" method="post">

<div class="icon-user-plus"></div>
<input type="text" name="username" value="<?php echo $datos['username']; ?>" placeholder="correo" required="requiered">
<input type="text" name="nombre" value="<?php echo $datos['nombre']; ?>" placeholder="nombre" required="requiered">
<input type="text" name="password"  value="<?php echo $datos['password']; ?>"placeholder="contraseña"   step="any" required="requiered">
<input type="number" name="rol" value="<?php  echo $datos['rol']?>"placeholder="rol"   step="any" required="requiered">
...
while ($row = $datos->fetch_array()) {
  ...
  echo '<td>',$row['rol'] ,'</td>';
	echo "<td> <a href=\"usuarios.php?accion=editar&&id=$row[idusuario]\">Editar</a> </td>";
	echo "<td> <a href=\"usuarios.php?accion=eliminar&&id=$row[idusuario]\">Eliminar</a> </td>";
